...
#define CMFINDER_PACKAGE_VERSION "0.4.1.15"
...
int main_cmfinder(ESL_GETOPTS *go) {
    ...
    if (needSeqFile) {
        ...
        if (seqfile == NULL) {
            cm_Fail("didn't get seqfile argument on commandline");
        }
    }
...
int main_summarize(ESL_GETOPTS *go) {
    ...
    printf("Num=%d\t Weight=%.2f\t Len=%.1f\t Score=%.2f\t Entropy=%.2f\t MI=%.2f\t BP=%.2f\t BP.org=%.2f\t Seq_id=%.2f\t Energy=%.2f\t GC=%.2f\t Conserved_pos=%d\t Conf_bp=%.2f\t Del_bp=%.2f\n",
            digital_msa->nseq, tot_weight, avg_seq_len, avg_score, e, m, avg_bppr, avg_bp, avg_seq_id, avg_energy, avg_GC, cons_pos, conf_bp, del_bp);
...
int main(int argc, char *argv[]) {
    ...
    if ((go = esl_getopts_Create(cmfinder_options)) == NULL) {
        cm_Fail("Internal failure creating options object");
    }
    if (esl_opt_ProcessEnvironment(go) != eslOK) {
        esl_fatal("Failed to process environment: %s\n", go->errbuf);
    }
