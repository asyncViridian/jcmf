...
test.data[, "Rank.index"] = sqrt(( test.data[, "Conserved_pos"] + 0.2) * test.data[, "BP"]/test.data[,"Seq_id"])*test.data[, "Species"]*(1+ log(test.data[,"Num"]/test.data[,"Species"]));
...
EOF
    close R;
    `/homes/gws/yzizhen/R/R-2.2.1/bin/R --no-save < $infile.r > /dev/null`;
};
